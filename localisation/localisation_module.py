...
class VisionEngine:
	def __init__(self, camera_index=0):
		# example ceiling markers hashmap
		CEILING_MAP = {
			0: (1.0, 1.0, 0.0),
			1: (2.0, 1.0, 0.0),
			5: (6.0, 1.0, 0.0)
	...
	def calculateCoordinates(self, corners, ids):
		...
		# will have to consider fish-eye effect to get position of the robot wrt the marker.
		pass
		...
		while self.running:
			...
			if ids is not None:
				...
				with self.lock:
					...
					self.currentMarker = ids[0]

	def getLatestPos(self):
		with self.lock:
			return self.centerPos

	def getLatestMarker(self):
		with self.lock:
			return self.currentMarker, self.direction # calc dir
